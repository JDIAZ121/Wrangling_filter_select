...
movies %>%
  arrange(year)
```
after running the codes,i found out that the longest movie was The cure for insomnia(5220 minutes).while the oldest movie was black was The Blackmith scene (1893).
...
movies_1995 <- movies %>%    
  filter(year == 1995)
```
After running the above code, i figured out that there were 1248 movies made in 1995.
...
```
After running the code,i found out that there were 4 movies that fall under those categories.
...
```
After running the code,i found out that there are 3 movies of Harry Potter included (The Chamber of secrets,The Prisoner of Azkaban,The Sorcerer'Stone). The movie with the highest rating was the Prisoner of Azkaban with a raing of 7.8,while the movie with the highest votes was the The Sorcerer's Stone with 46741 votes.
...
arrange (desc(budget))
```
After running the above code,i foound out the movie with the highest budget with 100 votes that was an animation or comedy was Wild Wild West with a budget of 170000000.
